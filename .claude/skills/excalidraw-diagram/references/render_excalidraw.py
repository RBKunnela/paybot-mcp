"""Render Excalidraw JSON to PNG and/or SVG using Playwright + headless Chromium.

Usage:
    cd .claude/skills/excalidraw-diagram/references
    uv run python render_excalidraw.py <path-to-file.excalidraw> [--output path.png] [--scale 2] [--width 1920]
    uv run python render_excalidraw.py <path-to-file.excalidraw> --svg          # SVG only
    uv run python render_excalidraw.py <path-to-file.excalidraw> --svg --png    # Both
...
def render(
    ...
    """Render an .excalidraw file to PNG and/or SVG. Returns the output PNG path (or SVG if svg_only)."""
...
def main() -> None:
    ...
    parser.add_argument("--output", "-o", type=Path, default=None, help="Output path (default: same name with .png)")
    ...
    parser.add_argument("--svg", action="store_true", help="Export SVG (use alone for SVG-only, combine with --png for both)")
    parser.add_argument("--png", action="store_true", help="Export PNG (default when no format flags given)")
    ...
    svg_only = args.svg and not args.png
    export_svg = args.svg

    out = render(args.input, args.output, args.scale, args.width, export_svg=export_svg, svg_only=svg_only)
    print(str(out))
