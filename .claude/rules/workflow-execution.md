...
#### Pre-Phase: AIDR Check (@architect) — CONDITIONAL
- **Trigger:** Story touches AI-specific concerns (agent design, model routing, token budgets, persona changes, handoff parameters)
- **Check:** (1) Existing AIDR covers this? → reference it. (2) Change contradicts an AIDR? → update AIDR first. (3) No AIDR exists? → create one alongside implementation.
- **Mode-dependent:** DEV = advisory (non-blocking) | STAGING/PROD = soft block (override with documented reason)
- **Reference:** `docs/architecture/aidr/INDEX.md` | See `aidr-awareness.md` rule for area-to-AIDR mapping
...
- **Quality Foundation:** Tests + docs MANDATORY (see `quality-foundation.md`)
- **Status:** Ready → InProgress

#### Phase 3.5: Quality Foundation Self-Check (@dev)
- **Gate:** `quality-foundation-checklist.md` — @dev self-check
- **VETO** if: missing tests, missing docs, coverage < 80%, skipped tests
- **Must pass BEFORE requesting QA**

#### Phase 4: QA Gate (@qa) — ENHANCED
- **Task:** `qa-gate.md`
- **12 quality checks** (see `quality-foundation.md` + `story-lifecycle.md`)
- **Minimum 2 passes:** First Pass → Fixes → Re-Test → Final Pass
- **Decision:** PASS (2+ passes) / CONCERNS / FAIL
- **WAIVED:** Never for tests/docs. Only perf/security edge cases with @architect approval.
- **Status:** InProgress → InReview → Done

#### Phase 4.5: Regression Verification (@qa)
- Full regression suite on affected areas
- **VETO** if regressions found → return to @dev
- Must pass BEFORE @devops push
